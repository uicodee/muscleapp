...
export default class EntityService {
    ...
    static async getNextLevel(initData: string | undefined, option: string): Promise<AxiosResponse<Entity>> {
        return api.get<Entity>(`api/entities/next-level/{entityType}?entity_type=${option}`, {headers: {Authorization: `Bearer ${initData}`}})
    }

    static async upgradeLevel(initData: string | undefined, option: string): Promise<AxiosResponse> {
        return api.post(`api/entities/next-level/{entityType}/upgrade?entity_type=${option}`, {}, {headers: {Authorization: `Bearer ${initData}`}})
    }
